...
**Phase 3 的最小 HTTP / API 入口已开始施工，但 `apps/web` 的 Next build 还未打通。**
...
### 当前卡点
- `apps/web` 的 Next build 还没通过
- 根因不是业务逻辑，而是：
  - monorepo package resolution 不统一
  - `src` / `dist` / Next bundling 三套解析链混在一起
  - `@obsidianhub/core` / `@obsidianhub/vault` / `@obsidianhub/config` 在 `apps/web` 构建期仍会被拉回错误入口

### 当前判断
- 这一阶段**不能算完成**
- 目前最需要的不是继续扩 API 动作，而是先**修通 `apps/web` + workspace packages 的 build 链路**
